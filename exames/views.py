...
def solicitar_exames(request):
    ...
    if request.method == "GET":
        ...
        #TODO: calcular total somente dos exames disponiveis
        preco_total = 0
...
def cancelar_pedido(request, pedido_id):
    ...
    if not pedido.usuario == request.user:
        messages.add_message(request, constants.ERROR, 'Voce não pode cancelar este pedido, Ele NÃO é seu!!')
        return redirect('/exames/gerenciar_pedidos/')
...
def permitir_abrir_exame(request, exame_id):
    ...
    if not exame.requer_senha:
        #TODO: verificar se tem pdf
        try:
            return redirect(exame.resultado.url)
        except:
            messages.add_message(request, constants.ERROR, 'Não existe o arquivo PDF, favor verificar!!')
    ...
    if request.method == "GET":
        ...
        if senha == exame.senha:
            ...
            try:
                ...
                messages.add_message(request, constants.ERROR, 'Não existe o arquivo PDF, favor verificar!!')
                return redirect(f'/exames/solicitar_senha_exame/{exame_id}')
        messages.add_message(request, constants.ERROR, 'Senha inválida, favor verificar!!')
        return redirect(f'/exames/solicitar_senha_exame/{exame_id}')
